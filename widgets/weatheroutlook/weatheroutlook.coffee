class Dashing.weatheroutlook extends Dashing.Widget
  ...
  onData: (data) ->
    @setBackgroundClassBy parseInt(data.forecasts[0].high, 10), data.forecasts[0].format

  setBackgroundClassBy: (temperature, format) ->
    @removeBackgroundClass()

    colorLevel = @findColorLevelBy temperature, format
    $(@node).addClass "weatheroutlook-temperature-#{colorLevel}"

  removeBackgroundClass: ->
    classNames = $(@node).attr("class").split " "

    for className in classNames
      match = /weatheroutlook-temperature-(.*)/.exec className
